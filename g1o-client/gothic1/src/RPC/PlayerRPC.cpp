...
void PlayerRPC::CreateAndSpawnPlayer(BitStream& stream)
{
	...
	CPlayer* player = playerManager.CreatePlayer(playerID, playerName);
		if( player )
		{
			...
			player->Spawn(player->pos[0], player->pos[1], player->pos[2], instanceName);
			if( player->npc != NULL )
			{
...
void PlayerRPC::ChangeInstance(BitStream& stream)
{
	...
	if( core.GetMultiplayer()->GetMyID() != playerID )
	{
		...
		if( player )
		{
			...
			oCNpc* new_npc = oCGame::GetGame()->CreateNPC(zSTRING(player->instance.C_String()), player->pos[0], player->pos[1], player->pos[2]);
			if( new_npc )
			{
				...
				new_npc->name[0] = zSTRING(RakString("%s (%d)", player->name.C_String(), player->GetID()).C_String());
				if( old_npc )
					old_npc->DestroyNpc();
			}
		}
...
void PlayerRPC::SetOverlay(BitStream& stream)
{
	...
	{
		if (add)
			player->npc->ApplyOverlay(zSTRING(overlay.C_String()));
		else
			player->npc->RemoveOverlay(zSTRING(overlay.C_String()));
	}
};
