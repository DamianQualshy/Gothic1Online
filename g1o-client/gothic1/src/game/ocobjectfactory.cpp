...
oCItem* oCObjectFactory::CreateItem2(zSTRING const& instance, zVEC3 pos, int amount)
{
	...
	if( index > 0 )
	{
		...
		if( item )
		{
			...
			oCNpc::GetHero()->_DoDropVob(item); //Musimy wywalic w taki sposob ale nie laguje
...
zCVob* oCObjectFactory::CreateVob(zSTRING const& visualName, int colldet, zVEC3 pos, zVEC3 rotation)
{
...
zCVob* oCObjectFactory::CreateMob(zSTRING const& visualName, int colldet, zVEC3 pos, zVEC3 rotation, unsigned long mobType)
{
	zCVob* vob = NULL;
	switch(mobType)
	{
	...
	case VOB_TYPE_VT_OCMOBWHEEL:
		vob = (zCVob*)oCMobWheel::_CreateNewInstance();
		break;
	default:
