...
void CVariable::RegisterConstants(g1o::script::NativeRegistry& registry)
{
	// Keys
	for (int i = 1; i <= KEYS_AMOUNT; ++i)
		registry.AddConstant(KEYS[i - 1], static_cast<int>(i));
		
	// Weapon modes
	for (int i = 0; i <= WEAPONMODES_AMOUNT; ++i)
		registry.AddConstant(WEAPONMODES[i], static_cast<int>(i));

	// Skill level
	for (int i = 0; i < SKILL_LEVEL_AMOUNT; ++i)
		registry.AddConstant(SKILL_LEVEL[i], static_cast<int>(i));

	// Skill level
	for (int i = 1; i <= SKILL_WEAPON_AMOUNT; ++i)
		registry.AddConstant(SKILL_WEAPON[i - 1], static_cast<int>(i));

	// Protection
	for (int i = 0; i < SKILL_WEAPON_AMOUNT; ++i)
		registry.AddConstant(PROTECTION_TYPE[i], static_cast<int>(i));
	...
	for (int i = 0; i < MOB_TYPE_AMOUNT; ++i)
		registry.AddConstant(MOB_TYPE[i], static_cast<int>(i));

	// Hand types
	for (int i = 0; i < 2; ++i)
		registry.AddConstant(HAND_TYPE[i], static_cast<int>(i));
}