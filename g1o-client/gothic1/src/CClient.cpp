...
CClient::CClient()
{
	new MemLib();
	new CLog();
	LOG("[startup] GO.dll client construction started");
	...
	LOG("[startup] GO.dll client components constructed");
};
...
void CClient::Start()
{
	LOG("[startup] Installing early Gothic hooks");
	DLOG("CClient::Start()");
	//Zmiana nazwy okna
	...
	LOG("[startup] GO.dll initialization completed");
	DLOG("Client started");
};
...
void CClient::InitializeHooks()
{
	...
	pMemLib->ImportHook(0x00429B80, sizeof(int(CGameManager::*)(zSTRING,int)), &CGameManager::Fake_PlayVideo); //Filmiki na początku gry
	pMemLib->ImportHook(0x0042A310, sizeof(int(*)()), &Hook_Apply_Options_Video); //Zmiana rozdzielczosci
	pMemLib->ImportHook(0x004279F0, sizeof(void(CGameManager::*)(int)), &CGameManager::Hook_Menu); //Hook na menu, "odczepia" się w innym miejscu
	pMemLib->ImportHook(0x0045D080, sizeof(zSTRING(zCOption::*)(const zSTRING&, const char*, const char*)), &zCOption::Hook_ReadString); //Tutaj zablokuję opcję "NOWA GRA" z menu
...
#ifndef DEV_MODE
	pMemLib->ImportHook(0x004301F0, sizeof(void(*)(zSTRING)), &Fake_HandleResultString); //Czity gothicowe (marvin, southpark etc.)
#endif //DEV_MODE
};
...
void CClient::DeInitializeHooks()
{
	...
	pMemLib->RemoveHook(0x00429B80); //filmiki na początku
...
#ifndef DEV_MODE
	pMemLib->RemoveHook(0x004301F0); //Gothicowe czity
#endif //DEV_MODE
};
...
void CClient::InitializeLateHooks()
{
	...
	pMemLib->ImportHook(0x0065EEE0, sizeof(int(oCGame::*)(int)), &oCGame::Hook_HandleEvent); //Hook na klawiaturę
	...
	pMemLib->ImportHook(0x006BB2F0, sizeof(void(oCNpc::*)(void)), &oCNpc::Hook_CloseInventory); //CloseInventory co tu dużo mówić?
	pMemLib->ImportHook(0x0068AD40, sizeof(int(oCNpc::*)(zSTRING const&)), &oCNpc::Hook_ApplyOverlay); // Przechwytywanie dodania overlay
	pMemLib->ImportHook(0x0068B040, sizeof(void(oCNpc::*)(zSTRING const&)), &oCNpc::Hook_RemoveOverlay); // Przechwytywanie usunięcia overlay
	...
	pMemLib->ImportHook(0x00741990, sizeof(void(oCNpc::*)(oCNpc::oSDamageDescriptor&)), &oCNpc::Hook_OnDamage_Anim);
	pMemLib->ImportHook(0x00738E40, sizeof(void(oCNpc::*)(oCNpc::oSDamageDescriptor&)), &oCNpc::Hook_OnDamage_Script);
...
void CClient::DeInitializeLateHooks()
{
	...
	pMemLib->RemoveHook(0x0065EEE0); //Hook na klawiaturę
	...
	pMemLib->RemoveHook(0x0068B040); //Przechwytywanie usunięcia overlay
