...
void CPlayer::Respawn()
{
	...
	if( this->npc )
	{
		...
		this->pos[0] = 0; this->pos[1] = -3000; this->pos[2] = 0;
...
void CPlayer::SetAdditionalVisuals(zSTRING const& _bodyModel, int _bodyTexture, zSTRING const& _headModel, int _headTexture)
{
		...
		if( isSlideAllowed )
		{
			if( npc )
			{
				if (!dead && npc->GetAttribute(NPC_ATR_HITPOINTS) > 0 && npc->IsUnconscious() == 0)
				{
					...
					float distance2d = goMath::GetDistance2D(_pos[0], _pos[2], pos[0], pos[2]);
					float distance3d = goMath::GetDistance3D(_pos[0], _pos[1], _pos[2], pos[0], pos[1], pos[2]);

					if( distance2d >= 1 && distance3d < 200 )
					{
						float _angle = goMath::GetVectorAngle(_pos[0], _pos[2], pos[0], pos[2]);
						_pos[0] += sin(_angle * 3.14f / 180.0f) * (distance3d / 15);
						_pos[2] += cos(_angle * 3.14f / 180.0f) * (distance3d / 15);

						if(pos[1] > _pos[1])
							_pos[1] += (distance3d / 30);
						else if(pos[1] < _pos[1])
							_pos[1] -= (distance3d / 30);

						npc->SetPosition(_pos[0], _pos[1], _pos[2]);
					}
