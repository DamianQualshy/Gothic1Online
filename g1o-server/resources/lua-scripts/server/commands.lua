-- Server command examples. Commands are entered without the leading slash here.
...
function exampleSendHelp(playerId)
    ...
    sendMessageToPlayer(playerId, 255, 210, 80, "/notify - trigger a custom client event")
end
...
function handlePlayerCommand(playerId, command, params)
    ...
    elseif command == "notify" then
        triggerClientEvent(playerId, EXAMPLE_EVENT_SERVER_WELCOME, getPlayerName(playerId), getPlayersCount())
    end
end
